...
async fn purge_chain_works() {
	...
	assert!(base_dir.path().join("pezkuwi/chains/rococo_local_testnet/db/full").exists());

	let status = Command::new(cargo_bin("pezkuwi-parachain"))
		.args(["purge-chain", "-d"])
		.arg(base_dir.path())
		.arg("-y")
		.status()
	...
	assert!(!base_dir.path().join("chains/local_testnet/db/full").exists());
	assert!(base_dir.path().join("pezkuwi/chains/rococo_local_testnet").exists());
	assert!(!base_dir.path().join("pezkuwi/chains/rococo_local_testnet/db/full").exists());
}
