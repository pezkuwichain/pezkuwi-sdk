...
pub fn pezkuwichain_development_config() -> Result<PezkuwiChainSpec, String> {
    ...
    let wrapper_account: AccountId = pezkuwi_runtime::TokenWrapperPalletId::get().into_account_truncating();
    ...
    let assets_config = AssetsConfig {
        assets: vec![
            (0, wrapper_account.clone(), true, 1),  // wHEZ - wrapper pallet owns
            (1, root_key.clone(), true, 1),         // PEZ
        ],
        metadata: vec![
            (0, "Wrapped HEZ".into(), "wHEZ".into(), 12),  // wHEZ metadata
            (1, "Pez".into(), "PEZ".into(), 12),           // PEZ metadata
        ],
        accounts: vec![
    ...
    let sudo_patch = serde_json::json!({
        "sudo": {
            "key": root_key.clone()
        }
    });
    ...
    let balances_patch = serde_json::json!({
        "balances": {
            "balances": vec![
                (alice_account.clone(), 10_000_000_000_000_000_000_000u128),
                (wrapper_account.clone(), 10_000_000_000_000_000_000_000u128),
            ]            
        }
    });
    ...
    if let serde_json::Value::Object(obj) = sudo_patch {
...
pub fn pezkuwichain_local_testnet_config() -> Result<PezkuwiChainSpec, String> {
    ...
    let assets_config = AssetsConfig {
        assets: vec![
            (0, wrapper_account.clone(), true, 1),  // wHEZ
            (1, alice_account.clone(), true, 1),    // PEZ
        ],
        metadata: vec![
            (0, "Wrapped HEZ".into(), "wHEZ".into(), 12),  // wHEZ
            (1, "Pez".into(), "PEZ".into(), 12),           // PEZ
        ],
        accounts: vec![
            // wHEZ starts at 0
    ...
    let balances_patch = serde_json::json!({
        "balances": {
            "balances": vec![
                ...
                (wrapper_account.clone(), ENDOWMENT),
            ]
        }
    });
