...
match $chain {
			...
			Chain::Rococo => Err("Rococo benchmark not supported"),
			Chain::PezkuwiChain => {
    			#[cfg(feature = "pezkuwi-native")]
    			{
        			use pezkuwichain as runtime;
    			...
    			{
        			Err("`pezkuwi-native` feature not enabled")
    			}
			},
...
/// Generates inherent data for benchmarking Pezkuwi, Kusama, Westend and Rococo.
///
...
pub fn benchmark_inherent_data(
	...
	inherent_data.put_data(pezkuwi_primitives::PARACHAINS_INHERENT_IDENTIFIER, &para_data)?;
