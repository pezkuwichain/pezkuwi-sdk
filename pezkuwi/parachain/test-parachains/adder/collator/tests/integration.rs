...
async fn collating_using_adder_collator() {
	...
	// start alice
	let alice = pezkuwi_test_service::run_validator_node(alice_config, Some(workers_path.clone()));
	...
	// start bob
	let bob = pezkuwi_test_service::run_validator_node(bob_config, Some(workers_path));
