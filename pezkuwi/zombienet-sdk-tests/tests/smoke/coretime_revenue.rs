...
//! `cargo build -r -F fast-runtime -p pezkuwi-parachain-bin && \`
...
use rococo::runtime_types::{
	pezkuwi_parachain_primitives::primitives,
	staging_xcm::v4::{
...
use subxt::{events::StaticEvent, utils::AccountId32, OnlineClient, PezkuwiConfig};
...
async fn coretime_revenue_test() -> Result<(), anyhow::Error> {
	...
	let config = NetworkConfigBuilder::new()
		...
		.with_parachain(|p| {
			p.with_id(1005)
				.with_default_command("pezkuwi-parachain")
