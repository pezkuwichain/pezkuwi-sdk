...
async fn spam_statement_distribution_requests_test() -> Result<(), anyhow::Error> {
	...
	honest
		.assert(
			"pezkuwi_parachain_approval_checking_finality_lag{chain=\"rococo_local_testnet\"}",
			0.0,
		)
		.await?;
	...
	honest
		.assert("pezkuwi_parachain_disputes_finality_lag{chain=\"rococo_local_testnet\"}", 0.0)
		.await?;
