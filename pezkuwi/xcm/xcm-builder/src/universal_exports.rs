...
mod tests {
	...
	fn ensure_is_remote_works() {
		// A Kusama parachain is remote from the Pezkuwi Relay.
		let x = ensure_is_remote(Pezkuwi, (Parent, Kusama, Parachain(1000)));
		assert_eq!(x, Ok((Kusama, Parachain(1000).into())));

		// Pezkuwi Relay is remote from a Kusama parachain.
		let x = ensure_is_remote((Kusama, Parachain(1000)), (Parent, Parent, Pezkuwi));
		assert_eq!(x, Ok((Pezkuwi, Here)));

		// Our own parachain is local.
		let x = ensure_is_remote(Pezkuwi, Parachain(1000));
		assert_eq!(x, Err(Parachain(1000).into()));

		// Pezkuwi's parachain is not remote if we are Pezkuwi.
		let x = ensure_is_remote(Pezkuwi, (Parent, Pezkuwi, Parachain(1000)));
		assert_eq!(x, Err((Parent, Pezkuwi, Parachain(1000)).into()));

		// If we don't have a consensus ancestor, then we cannot determine remoteness.
		let x = ensure_is_remote((), (Parent, Pezkuwi, Parachain(1000)));
		assert_eq!(x, Err((Parent, Pezkuwi, Parachain(1000)).into()));
	}
