...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let id = if id == "" {
			...
			["pezkuwi", "kusama", "westend", "rococo", "versi"]
				.iter()
				.cloned()
		...
		Ok(match id {
			...
			#[cfg(feature = "pezkuwi-native")]
			"pezkuwichain-alfa-testnet" => Box::new(pezkuwi_service::chain_spec::pezkuwichain_alfa_testnet_config()?),
			#[cfg(feature = "pezkuwi-native")]
			"pezkuwichain-beta-testnet" => Box::new(pezkuwi_service::chain_spec::pezkuwichain_beta_testnet_config()?),
			#[cfg(feature = "pezkuwi-native")]
			"pezkuwichain-staging" => Box::new(pezkuwi_service::chain_spec::pezkuwichain_staging_config()?),
			#[cfg(feature = "pezkuwi-native")]
			"pezkuwichain-production" | "pezkuwichain" => Box::new(pezkuwi_service::chain_spec::pezkuwichain_production_config()?),
			#[cfg(feature = "pezkuwi-native")]
			...
			#[cfg(feature = "rococo-native")]
			"rococo-staging" => Box::new(pezkuwi_service::chain_spec::rococo_staging_testnet_config()?),
			#[cfg(not(feature = "rococo-native"))]
			name if name.starts_with("rococo-") && !name.ends_with(".json") =>
				Err(format!("`{}` only supported with `rococo-native` feature enabled.", name))?,
			"westend" => Box::new(pezkuwi_service::chain_spec::westend_config()?),
			#[cfg(feature = "westend-native")]
			"westend-dev" => Box::new(pezkuwi_service::chain_spec::westend_development_config()?),
			#[cfg(feature = "westend-native")]
			"westend-local" => Box::new(pezkuwi_service::chain_spec::westend_local_testnet_config()?),
			#[cfg(feature = "westend-native")]
			"westend-staging" => Box::new(pezkuwi_service::chain_spec::westend_staging_testnet_config()?),
			#[cfg(feature = "rococo-native")]
			...
			path => {
					...
					chain_spec.is_versi()
				{
					...
					Box::new(pezkuwi_service::WestendChainSpec::from_json_file(path)?)
				} else {
					chain_spec
				}
			},
...
fn set_default_ss58_version(_spec: &Box<dyn pezkuwi_service::ChainSpec>) {
	let ss58_version = Ss58AddressFormat::custom(42);
	...
	runner.run_node_until_exit(move |config| async move {
		...
		let task_manager = pezkuwi_service::build_full(
			...
			pezkuwi_service::NewFullParams {
				is_parachain_node: pezkuwi_service::IsParachainNode::No,
				enable_beefy,
