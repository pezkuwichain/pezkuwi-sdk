...
use pezkuwichain_constants::system_parachain::{coretime::TIMESLICE_PERIOD, BROKER_ID};
...
impl_runtime_weights!(pezkuwichain_constants);
...
parameter_types! {
	pub const KycApplicationDepositAmount: Balance = 10 * UNITS;
}
...
impl EnsureOriginWithArg<RuntimeOrigin, RuntimeParametersKey> for DynamicParameterOrigin {
	...
	fn try_successful_origin(_key: &RuntimeParametersKey) -> Result<RuntimeOrigin, ()> {
		<EnsureRoot<AccountId> as frame_support::traits::EnsureOrigin<RuntimeOrigin>>::try_successful_origin()
	}
}
...
impl parachains_configuration::Config for Runtime {
	type WeightInfo = weights::pezkuwi_runtime_parachains_configuration::WeightInfo<Runtime>;
}
...
impl pezkuwi_runtime_parachains::inclusion::RewardValidators for RewardValidators {
...
impl parachains_inclusion::Config for Runtime {
	...
	type WeightInfo = weights::pezkuwi_runtime_parachains_inclusion::WeightInfo<Runtime>;
}
...
impl parachains_paras::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = weights::pezkuwi_runtime_parachains_paras::WeightInfo<Runtime>;
...
impl parachains_hrmp::Config for Runtime {
	...
	type WeightInfo = weights::pezkuwi_runtime_parachains_hrmp::WeightInfo<Runtime>;
}

impl parachains_paras_inherent::Config for Runtime {
	type WeightInfo = weights::pezkuwi_runtime_parachains_paras_inherent::WeightInfo<Runtime>;
}
...
impl coretime::Config for Runtime {
	...
	type WeightInfo = weights::pezkuwi_runtime_parachains_coretime::WeightInfo<Runtime>;
...
impl parachains_on_demand::Config for Runtime {
	...
	type WeightInfo = weights::pezkuwi_runtime_parachains_on_demand::WeightInfo<Runtime>;
...
impl parachains_initializer::Config for Runtime {
	type Randomness = pallet_babe::RandomnessFromOneEpochAgo<Runtime>;
	type ForceOrigin = EnsureRoot<AccountId>;
	type WeightInfo = weights::pezkuwi_runtime_parachains_initializer::WeightInfo<Runtime>;
...
impl parachains_disputes::Config for Runtime {
	...
	type WeightInfo = weights::pezkuwi_runtime_parachains_disputes::WeightInfo<Runtime>;
}
...
mod benches {
	frame_benchmarking::define_benchmarks!(
		...
		[pezkuwi_runtime_parachains::configuration, Configuration]
		[pezkuwi_runtime_parachains::coretime, Coretime]
		[pezkuwi_runtime_parachains::hrmp, Hrmp]
		[pezkuwi_runtime_parachains::disputes, ParasDisputes]
		[pezkuwi_runtime_parachains::inclusion, ParaInclusion]
		[pezkuwi_runtime_parachains::initializer, Initializer]
		[pezkuwi_runtime_parachains::paras_inherent, ParaInherent]
		[pezkuwi_runtime_parachains::paras, Paras]
		[pezkuwi_runtime_parachains::on_demand, OnDemandAssignmentProvider]
		// Substrate
...
sp_api::impl_runtime_apis! {
				...
				Ok(asset_id) if asset_id.0 == xcm_config::TokenLocation::get() => {
					...
					Ok(
						<frame_support::weights::ConstantMultiplier<Balance, TransactionByteFee>
							as frame_support::weights::WeightToFee>::weight_to_fee(&weight)
					)
				},
				Ok(asset_id) => {
	...
	#[api_version(13)]
	impl pezkuwi_primitives::runtime_api::ParachainHost<Block> for Runtime {
		...
		fn para_backing_state(para_id: ParaId) -> Option<pezkuwi_primitives::vstaging::async_backing::BackingState> {
			#[allow(deprecated)]
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		...
		> {
			...
			parameter_types! {
				...
				pub AssetHubParaId: ParaId = pezkuwichain_constants::system_parachain::ASSET_HUB_ID.into();
