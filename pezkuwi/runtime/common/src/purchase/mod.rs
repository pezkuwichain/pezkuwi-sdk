...
//! Pallet to process purchase of HEZs.
...
/// All information about an account regarding the purchase of HEZs.
...
pub struct AccountStatus<Balance> {
	...
	/// The amount of free HEZs they have purchased.
	free_balance: Balance,
	/// The amount of locked HEZs they have purchased.
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The amount of purchased locked HEZs that we will unlock for basic actions on the chain.
		#[pallet::constant]
		type UnlockedProportion: Get<Permill>;

		/// The maximum amount of locked HEZs that we will unlock.
		#[pallet::constant]
	...
	impl<T: Config> Pallet<T> {
			...
			Accounts::<T>::try_mutate(
				...
				|status: &mut AccountStatus<BalanceOf<T>>| -> DispatchResult {
					...
					if !status.locked_balance.is_zero() {
						...
						// We allow some configurable portion of the purchased locked HEZs to be
		...
		/// Set the block where locked HEZs will become unlocked.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[pallet::call_index(6)]
		#[pallet::weight(T::DbWeight::get().writes(1))]
