...
pub use pallet::*;
use pezkuwi_runtime_parachains::paras::{OnNewHead, ParaKind};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule_code_upgrade(
			...
			Self::ensure_root_para_or_owner(origin, para)?;
			pezkuwi_runtime_parachains::schedule_code_upgrade::<T>(
				para,
				new_code,
		...
		pub fn set_current_head(
			...
			Self::ensure_root_para_or_owner(origin, para)?;
			pezkuwi_runtime_parachains::set_current_head::<T>(para, new_head);
			Ok(())
		}
	}
}
...
impl<T: Config> Registrar for Pallet<T> {
	...
	fn make_parachain(id: ParaId) -> DispatchResult {
		...
		pezkuwi_runtime_parachains::schedule_parathread_upgrade::<T>(id)
	...
	fn make_parathread(id: ParaId) -> DispatchResult {
		...
		pezkuwi_runtime_parachains::schedule_parachain_downgrade::<T>(id)
	...
	fn execute_pending_transitions() {
		use pezkuwi_runtime_parachains::shared;
...
impl<T: Config> Pallet<T> {
	...
	fn do_register(
		...
		let res = pezkuwi_runtime_parachains::schedule_para_initialize::<T>(id, genesis);
	...
	fn do_deregister(id: ParaId) -> DispatchResult {
		...
		pezkuwi_runtime_parachains::schedule_para_cleanup::<T>(id)
	...
	fn do_thread_and_chain_swap(to_downgrade: ParaId, to_upgrade: ParaId) {
		let res1 = pezkuwi_runtime_parachains::schedule_parachain_downgrade::<T>(to_downgrade);
		debug_assert!(res1.is_ok());
		let res2 = pezkuwi_runtime_parachains::schedule_parathread_upgrade::<T>(to_upgrade);
