...
//! # Example
//!
//! Example of opening a bridge between some random parachains from Pezkuwi and Kusama:
//!
//! 0. Let's have:
//! 	- BridgeHubPezkuwi with `UniversalLocation` = `[GlobalConsensus(Pezkuwi), Parachain(1002)]`
//! 	- BridgeHubKusama with `UniversalLocation` = `[GlobalConsensus(Kusama), Parachain(1002)]`
//! 1. The Pezkuwi local sibling parachain `Location::new(1, Parachain(1234))` must send some HEZs
//!    to its sovereign account on BridgeHubPezkuwi to cover `BridgeDeposit`, fees for `Transact`,
//!    and the existential deposit.
//! 2. Send a call to the BridgeHubPezkuwi from the local sibling parachain: `Location::new(1,
...
//!    VersionedInteriorLocation::V4([GlobalConsensus(Pezkuwi), Parachain(1234)].into()), ); ) ```
